...
SELECT
	...
	percentile_cont(0.50) within group (order by valor) as segundo_quartil,
...
SELECT
    ...
    M.media_movel_3dias,
    DENSE_RANK() OVER (
        PARTITION BY M.data_lancamento
        ORDER BY M.media_movel_3dias DESC
    ) AS rank_media_movel
FROM
    LancamentosOrdenados M
ORDER BY
    M.data_lancamento, rank_media_movel;